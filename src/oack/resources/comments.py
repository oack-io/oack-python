...
class AsyncComments:
    ...
    async def create(self, team_id: str, monitor_id: str, body: str) -> Comment:
        resp = await self._client.request(
            "POST", _comment_path(team_id, monitor_id), json={"body": body}
        )
    ...
    async def reply(
        self, team_id: str, monitor_id: str, comment_id: str, body: str
    ) -> Comment:
        resp = await self._client.request(
            "POST",
    ...
    async def list_replies(
        self, team_id: str, monitor_id: str, comment_id: str
    ) -> list[Comment]:
        resp = await self._client.request(
            "GET", _comment_path(team_id, monitor_id) + f"/{comment_id}/replies"
        )
        return [Comment.model_validate(c) for c in json.loads(resp)]

    async def resolve(self, team_id: str, monitor_id: str, comment_id: str) -> Comment:
        resp = await self._client.request(
            "POST", _comment_path(team_id, monitor_id) + f"/{comment_id}/resolve"
        )
        return Comment.model_validate_json(resp)

    async def reopen(self, team_id: str, monitor_id: str, comment_id: str) -> Comment:
        resp = await self._client.request(
            "POST", _comment_path(team_id, monitor_id) + f"/{comment_id}/reopen"
        )
        return Comment.model_validate_json(resp)
    ...
    async def list_edits(
        ...
        resp = await self._client.request(
            "GET", _comment_path(team_id, monitor_id) + f"/{comment_id}/edits"
        )
